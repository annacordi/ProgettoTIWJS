...
public class VerbaleDAO {
	...
	// cerca id degli studenti pubblicati o rifiutati
	public List<Integer> cercaIdStudentiPubbORif() throws SQLException {
		List<Integer> idList = new ArrayList<Integer>();
		String query = "SELECT u.id " + "FROM esame e " + "JOIN user u ON u.id = e.idstudente "
	...
	// riceve una lista di id degli studenti e prende le informazioni per ognuno di
	// essi
	public List<InfoIscritti> infoStudentiAggiornati(int appId, List<Integer> studentIds) throws SQLException {
		...
		String query = "SELECT e.idstudente, e.voto, e.statodivalutazione, u.nome, u.cognome, u.matricola "
				+ "FROM esame e JOIN user u ON e.idstudente = u.id " + "WHERE e.idapp = ? AND e.idstudente IN (" + inSql
				+ ")";

		try (PreparedStatement pstm = con.prepareStatement(query)) {
			pstm.setInt(1, appId);
			for (int i = 0; i < studentIds.size(); i++) {
				pstm.setInt(i + 2, studentIds.get(i)); // +2 perchè il primo parametro è appId ogni volta
			}
			...
			try (ResultSet rs = pstm.executeQuery()) {
				...
				while (rs.next()) {
					...
					info.setStatodivalutazione(
							Statodivalutazione.valueOf(rs.getString("statodivalutazione").toUpperCase()));
	...
	// crea un nuovo verbale con id autoincrementato
	public void creaverbale() throws SQLException {
		String query = "INSERT INTO verbale (data, ora, idapp) " + "VALUES (CURRENT_DATE(), CURRENT_TIME(), ?);";
		try (PreparedStatement pstatement = con.prepareStatement(query);) {
			pstatement.setInt(1, this.idapp);
			pstatement.executeUpdate();
		}
	...
	// selezioni i dati dell'ultimo verbale creato
	public Verbale idVerb() throws SQLException {
		...
		String query = "SELECT v.idverb, v.data AS verbale_data, v.ora, a.idapp, a.data AS appello_data "
				+ "FROM verbale v " + "JOIN appello a ON v.idapp = a.idapp "
